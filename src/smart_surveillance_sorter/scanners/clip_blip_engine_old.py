...
class ClipBlipEngine:
    ...
    def scan_single_video(self,video_data):
        ...
        for frame in frames:
            ...
            #     best_class = "PERSON"
            # Recuperiamo il valore (es. 1.1)
            fake_rel = active_rules.get("PERSON_FAKE_RELATIVE", self.PERSON_FAKE_RELATIVE)

            # Primo controllo modificato
            # Se (PERSONA * 1.1) + TOLLERANZA > FAKE, allora passa.
            if final_scores["PERSON"] >= self.PERSON_PRIORITY_THRESHOLD and \
            max_fake_score < (final_scores["PERSON"] * fake_rel + self.PERSON_BOOST_TOLERANCE):
                    best_class = "PERSON"
            elif(is_night):
    ...
    def _calculate_day_category(
        ...
        fake_rel = rules.get("PERSON_FAKE_RELATIVE", self.PERSON_FAKE_RELATIVE)
    
        if best_score_cls == "PERSON":
            if max_fake_score > self.VETO_THRESHOLD:
                delta_threshold = self.DELTA_THRESHOLD.get("PERSON", 0.25)
                # RIPRISTINO LOGICA: Il fake score viene "pesato" dal relative
                # Se la persona è 0.1 e il fake è 0.105, con fake_rel 1.1 la persona vince.
                if (best_score_val * fake_rel - max_fake_score) > delta_threshold:
                    return "PERSON"
        # if best_score_cls == "PERSON":
