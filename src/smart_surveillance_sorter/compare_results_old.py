...
def compare_results(session_dir=None, gt_file=None, res_file=None, log=None):
    ...
    _print(f"Video nel GT:       {len(gt_map)}")
    _print(f"Video nei risultati: {len(res_map)}")
    _print(f"Video confrontati:  {len(all_videos)}")
    if only_in_gt:
        _print(f"⚠️  {len(only_in_gt)} video nel GT ma non nei risultati (non processati?)")
    if only_in_res:
        _print(f"⚠️  {len(only_in_res)} video nei risultati ma non nel GT")
    _print("-" * 70)
